...
def main(bug_0, bug_1, bug_2):
    # Define obstacles and boundary
    o_x, o_y = [], []

    # Define rectangular boundary (top and bottom)
    for i in range(0, 101):  # Horizontal boundaries (top and bottom)
        o_x.append(i)
        o_y.append(0)  # Bottom boundary
        o_x.append(i)
        o_y.append(100)  # Top boundary

    for j in range(0, 101):  # Left boundary
        o_x.append(0)  # Left boundary
        o_y.append(j)

    # Right boundary divided into two segments
    for j in range(50, 90):  # Upper segment of right boundary
        o_x.append(100)
        o_y.append(j)

    # Define vertical barriers inside the rectangle
    for i in range(50, 95):  # First vertical wall
        o_x.append(15)
        o_y.append(i)

    for i in range(0, 60):  # Second vertical wall
        o_x.append(30)
        o_y.append(i)

    for i in range(30, 90):  # Third vertical wall
        o_x.append(45)
        o_y.append(i)

    for i in range(0, 60):  # Fourth vertical wall
        o_x.append(60)
        o_y.append(i)
    
    for i in range(30, 90):  # Fourth vertical wall
        o_x.append(75)
        o_y.append(i)

    # Start and goal points (visual markers)
    s_x = 5.0
    s_y = 50.0
    g_x = 98.0
    g_y = 60.0
